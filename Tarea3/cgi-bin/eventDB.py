...
class Event:
    ...
    # RETORNA LA CANTIDAD DE FOTOS QUE TIENE UN EVENTO
    def getNumberPhotos(self, id):
        self.cursor.execute(f'SELECT COUNT(evento_id) from foto where evento_id = {id}')
        return self.cursor.fetchall()

    # RETORNA LA CANTIDAD DE EVENTOS QUE HAY EN LOS DISTITNTOS DIAS
    def getEventsPerDay(self):
        self.cursor.execute(f" select DATE_FORMAT(dia_hora_inicio, '%Y-%m-%d') as fecha, count(*) as total from evento group by DATE_FORMAT(dia_hora_inicio, '%Y-%m-%d') order by DATE_FORMAT(dia_hora_inicio, '%Y-%m-%d') asc")
        return self.cursor.fetchall()

    # RETORNA LA CANTIDAD DE TIPOS DE EVENTOS QUE HAY GUARDADOS
    def getTypeEvents(self):
        self.cursor.execute('SELECT tipo, COUNT(*) FROM evento GROUP BY tipo')
        return self.cursor.fetchall()

    def getMapEvents(self):
        self.cursor.execute('SELECT *, COUNT(*) FROM evento GROUP BY comuna_id')
        return self.cursor.fetchall()
