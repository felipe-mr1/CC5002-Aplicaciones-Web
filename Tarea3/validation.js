...
function AddPhoto(){
    ...
    if (F<4){
        container.innerHTML += "<div class='entrada'><input name='foto-comida' id='foto-comida" + F + "' type='file' accept='image/png, image/jpg'></div>";
        F++;
    }
}
...
function socialNetworks(){
    ...
    if (R<4){
        container.innerHTML += "<div class='entrada'> <select id='red-social' name='red-social'>" +
            ...
            "</select><input type='text' id='red-social-url' name='red-social-url'></div>";
        R++;
    }
}
...
function getComuna(){
    ...
    for(let i = 0; i < chile.regiones[value].comunas.length; i++){
        selector.innerHTML += "<option value='" + chile.regiones[value].comunas[i] + "'>" + chile.regiones[value].comunas[i]  +"</option>";
    }
}
...
function notifyEvent(){
    ...
    for (let j = 0; j < comidas.length; j++){
        food.innerHTML += "<option value='" + comidas[j] + "'>" + comidas[j] + "</option>";
    }
}
...
function pagination(allEvents){
    let cointainer = document.getElementById("testeo");
    cointainer.innerHTML+= "<p>Hola test test</p>";
}

function pagination2(allEvents){
    let container = document.getElementById("testeo");
    container.innerHTML = "<table class='my_table'>"+
    "<tr>"+
        ...
        "<th>Total fotos</th>"+
    "</tr>"
    for(let j=0; allEvents.length; j++){
        container.innerHTML+="<tr>"+
            "<th>"+ str(allEvents[1]) + "</th>"+
            "<th>"+ str(allEvents[2]) + "</th>"+
            "<th>comuna</th>"+
            "<th>"+ str(allEvents[4]) + "</th>"+
            "<th>"+ str(allEvents[5]) + "</th>"+
            "<th>red_social</th>"+
            "<th>1</th>"+
        "</tr>"
    }
...
function EventoInfo(info){
    let container = document.getElementById("eventInfo")
    console.log(info)
    container.style.display = "block";
    container.innerHTML = "<h2>Información del evento:</h2><div class='leyenda'>"+
    "<img class='clickable' onclick='showPic(this.src)' src='" + info + "' width='320' height='240' alt='photo of food'>" +
...
function details(data){
    ...
    container.innerHTML = "<h2>" + infoEvent[9] + ": " + infoEvent[8] + "</h2>"+
    "<h2>Contacto: "+ infoEvent[3] + "/" + infoEvent[5] +"</h2>"+
    "<h2>Comuna: "+ infoEvent[1] + "/ Sector: " + infoEvent[2] +"</h2>"+
    "<h2>Foto: <image src = '../media/"+ infoEvent[0] +"' width='320' height='240' alt='foto comida'></h2>"+
